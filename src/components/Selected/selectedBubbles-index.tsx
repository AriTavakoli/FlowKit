...
export default function SelectedTemplates({accessType, sequenceRef} : SelectedTemplatesProps) {
  ...
  useEffect(() => {
    (async () => {
      const templates = await StorageOps.getAllStorageItemsByAccessType(accessType);
      const nodeTemplates = await StorageOps.getAllTemplatesForNode(sequenceRef.getId())


      console.log('%cnodeTemplates', 'color: lightblue; font-size: 44px', nodeTemplates);
      const newFragList = nodeTemplates ? Object.keys(nodeTemplates).map((template) => {
        return {
          uuid: uuidv4(),
